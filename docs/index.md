...
Mode 7 is a pseudo-3D technique from the retro era. A 2D tile map is rendered with perspective transformation â€” each scanline is scaled and offset based on its distance from the "camera," creating the illusion of a 3D floor receding into the distance.
